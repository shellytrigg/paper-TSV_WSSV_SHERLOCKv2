---
title: "2022-08-30_SHERLOCKv2_fluor_analyze_all"
...
- Make sure Rox was not included as a passive refernece. Under Setup -> Define -> Passive Reference (should say "None"). 
...
QS_data %>% filter(!grepl("04-21|04-27", date)) %>% filter(virus != "TSV" | date != "04-20") %>% filter(sample_type == "STD" & virus == "TSV") %>% filter(CT !="Undetermined") %>% mutate(template = log10(as.numeric(template))) %>% mutate(CT = as.numeric(CT)) %>% filter(template > 0) %>%ggplot(aes(x = template, y = CT))+ geom_point(aes(shape = virus),position = position_dodge(width = 0.5)) + theme_bw() + labs(color = "date", y = "Ct", x = "copies") +  ggtitle("standards from TSV different experiments") + facet_wrap(~date, scale = "free") + geom_smooth(size = 0.5, linetype = 3,method = "lm", se = F)+ stat_regline_equation(label.x = 3, label.y.npc = "top" ,aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~~")), formula = y~ x, size = 3)
dev.off()

#### WSSV ####
jpeg("../intermediate_files/WSSV_stdcrvs_before_outlier_removal.jpg", width = 10, height = 6, units = "in", res = 300)
QS_data %>% filter(!grepl("04-21|04-27", date)) %>% filter(virus != "TSV" | date != "04-20") %>% filter(sample_type == "STD" & virus == "WSSV") %>% filter(CT !="Undetermined") %>% mutate(template = log10(as.numeric(template))) %>% mutate(CT = as.numeric(CT)) %>% filter(template > 0) %>%ggplot(aes(x = template, y = CT))+ geom_point(aes(shape = virus),position = position_dodge(width = 0.5)) + theme_bw() + labs(color = "date", y = "Ct", x = "copies") +  ggtitle("standards from different WSSV experiments") + facet_wrap(~date, scale = "free") + geom_smooth(size = 0.5, linetype = 3,method = "lm", se = F)+ stat_regline_equation(label.x = 5,aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~~")), formula = y~ x, size = 3)
dev.off()
...
QS_STD_data_stats <- QS_data %>% filter(!grepl("04-21|04-27", date)) %>% filter(virus != "TSV" | date != "04-20") %>% filter(sample_type == "STD") %>% filter(CT !="Undetermined") %>% mutate(CT = as.numeric(CT)) %>% filter(template > 0) %>% group_by(date, virus,template,num_reps)%>% summarise(FivNum = list(fivenum(CT)),mean = mean(CT), sd = sd(CT), count = sum(!is.na(CT)), mad_CT = mad(CT)) %>% mutate(FivNum = lapply(FivNum,setNames,nm = c("min", "FirstQ", "median","ThirdQ", "max"))) %>% unnest_wider(FivNum) %>% filter(count >=num_reps/2) 
...
QS_STD_data <- merge(QS_data %>% filter(!grepl("04-21|04-27", date)) %>% filter(virus != "TSV" | date != "04-20") %>% filter(sample_type == "STD") %>% filter(CT !="Undetermined") %>%  mutate(CT = as.numeric(CT)) %>% filter(template > 0) ,QS_STD_data_stats, by = c("date", "template","virus","num_reps"))
...
QS_data_LOD <- QS_data_filtered %>% filter(grepl("02-09|12-21|03-24|04-22|06-23",date)) %>% filter(!grepl("Val",template)) %>% filter(sample_type == "UNK") %>% mutate(`Sample Name` = gsub("_VP28.31t","",`Sample Name`)) %>% mutate(copies = gsub(".*_", "", `Sample Name`))
...
jpeg("standardCTmeans_x_actual_copies.jpg", width = 5, height = 7, units = "in", res = 300)
...
```{r}
jpeg("subset_standardCTmeans_x_actual_copies.jpg", width = 7, height = 3, units = "in", res = 300)
...
jpeg("specificity.jpg", width = 7, height = 3, units = "in", res = 300)
...
dil_data_WSSV <- read_xlsx("../SHERLOCKv2_WSSV_validation_SAT_2022.xlsx", skip = 1,col_names = T)
...
dil_data_TSV <- read_xlsx("../TSV_dil_data.xlsx",sheet = "SHERLOCK",col_names = T)
...
TSV_qPCR <- read_xlsx("../TSV RNA from ArunDhar 20201124.xlsx", col_names = T, skip = 3,sheet = "Tech Rep Data - AD")
...
TSV_qPCR_SRM <- read.csv("../2022-10-28_TSV_qPCR_SRM/TSV_qPCR_SRM.csv")
#create new columns that will match with other data frames later on
...
jpeg("TSV_qPCR_suscept_v_resist.jpg", width = 8, height = 4, units = "in", res = 300)
...
jpeg("TSV_qPCR_suscept_v_resist_SRM.jpg", width = 4, height = 3, units = "in", res = 300)
...
```{r}
WSSV_qPCR <- read_xlsx("../../../../../Tim Sullivan Data/Experiments_Projects/Diagnostics/results/quant_master_data.xlsx", col_names = T)
...
jpeg("TSV_qPCR_SRMxSHERLOCKv2.jpg", width = 4, height = 3.5, units = "in", res = 300)
...
#### plot susceptible vs. resistant for just SRM data ####
jpeg("TSV_qPCR_suscept_v_resist_SRMqPCRvSHERLOCK.jpg", width = 4.5, height = 4, units = "in", res = 300)
...
```{r}
jpeg("WSSV_qPCRxSHERLOCKv2.jpg", width = 4, height = 3.5, units = "in", res = 300)
...
jpeg("WSSV_SHERLOCKv1xSHERLOCKv2.jpg", width = 4, height = 3.5, units = "in", res = 300)
...
write.csv(tableS3, "TableS3.csv", row.names = F, quote = F)
```
...
```{r}
jpeg("WSSV_SHERLOCKv1xSHERLOCKv2_CoV.jpg", width = 5, height = 4, units = "in", res = 300)
...
```{r}
jpeg("qPCRvsSHERLOCKv2_ViralCopiesFC.jpg", width = 4.5, height = 4, units = "in", res = 300)
